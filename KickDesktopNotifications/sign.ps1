...
if(Test-Path -Path 'KickNotify.zip'){ 
	Remove-Item 'KickNotify.zip'; 
}

# Files to be signed
$files = 'KickNotify.exe',
	 'KickNotify.dll';
...
Get-ChildItem './' | where { $_.Name -notin 'sign.ps1'} | Compress-Archive -DestinationPath 'KickNotify.zip' -Update
